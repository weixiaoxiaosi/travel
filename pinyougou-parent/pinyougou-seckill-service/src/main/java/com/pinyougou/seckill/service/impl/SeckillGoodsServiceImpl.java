	...
	**/
	...
	public List<TbSeckillGoods> findList() {
		...
		if (seckillGoodsList==null||seckillGoodsList.size()==0){
			Example example = new Example(TbSeckillGoods.class);
			Example.Criteria criteria = example.createCriteria();
			criteria.andEqualTo("status","1" );//审核状态的商品
			criteria.andGreaterThan("stockCount", 0);//大于0的库存商品
			Date now = new Date();
			criteria.andGreaterThanOrEqualTo("endTime",now );//大于等于秒杀开始时间的商品
			criteria.andLessThanOrEqualTo("startTime", now);//小于等于秒杀结束时间的商品
